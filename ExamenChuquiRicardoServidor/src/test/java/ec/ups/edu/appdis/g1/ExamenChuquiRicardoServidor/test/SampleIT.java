package ec.ups.edu.appdis.g1.ExamenChuquiRicardoServidor.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/ExamenChuquiRicardoServidor.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "ExamenChuquiRicardoServidor.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.ups.edu.appdis.g1.ExamenChuquiRicardoServidor.test");
